...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .desc { margin-top: 8px; font-size: 12px; color: #333; font-weight: 600; }
        @media print {
...
<body>
    ...
    <div class="info">
        <span>No: <strong>{{ $order->receipt_id }}</strong></span>
        <span>Tgl: {{ $order->date->isoFormat('D MMM YYYY  HH:mm') }}</span>
        <span>Akun: {{ $order->account->name ?? '-' }}</span>
    </div>
    <div class="divider"></div>
    <table>
        <tr>
            <td colspan="2" class="col-header">Layanan</td>
        ...
        <tr>
            <td colspan="2">@ Rp {{ number_format($order->price_per_unit, 0, ',', '.') }}/lbr</td>
            <td class="right"></td>
        </tr>
        <tr>
            <td colspan="2" style="text-align:right;font-weight:900;padding-top:6px;">Total</td>
            <td class="right" style="font-weight:900;border-top:2px solid #000;padding-top:6px;">{{ number_format($order->total, 0, ',', '.') }}</td>
        </tr>
    ...
    <div class="no-print" style="text-align:center;margin-top:15px;">
        <a href="{{ route('print-orders.receipt.pdf', $order->id) }}" target="_blank" style="display:inline-block;padding:10px 28px;border:none;border-radius:8px;background:#3b82f6;color:#fff;font-weight:700;text-decoration:none;margin-bottom:8px;font-size:14px;">
